...
const DashboardClient = ({ userData, applicationCountToday, jobApplicationCounts }) => {
  ...
  const totalApplications = jobApplicationCounts.reduce((total, job) => total + job.count, 0);
    ...
    <div className="min-h-screen flex flex-col items-center mt-12 p-4 md:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full max-w-4xl mb-4">
        ...
        <Card className="shadow-md hover:shadow-lg transition-shadow p-2 bg-white flex items-center rounded-[15px]">
          ...
          <CardHeader className="flex-1 flex flex-col ml-2">
            <CardTitle className="text-sm font-semibold">Welcome, {userData.name}</CardTitle>
          </CardHeader>
        ...
        <Card className="shadow-md hover:shadow-lg transition-shadow p-2 bg-white flex items-center rounded-[15px]">
          ...
          <CardHeader className="flex-1 flex flex-col ml-2">
            <CardTitle className="text-sm font-semibold">Applications Received Today</CardTitle>
          </CardHeader>
          <CardContent className="text-3xl font-bold text-gray-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full max-w-4xl">
        {reversedJobApplicationCounts.map((jobApp, index) => (
          <Card key={jobApp.jobId} className="shadow-md p-4 hover:shadow-lg transition-shadow  bg-white flex items-center rounded-[15px]">
            <Briefcase className="text-purple-500" size={24} /> {/* Icon */}
            <CardHeader className="flex-1 flex flex-col ml-2">
              <CardTitle className="text-sm font-semibold">{`Job ${index + 1}: ${jobApp.jobTitle}`}</CardTitle>
            </CardHeader>
            <CardContent className="text-3xl font-bold text-gray-700">
              {jobApp.count} 
            </CardContent>
