...
export async function POST(request) {
  try {
    ...
    const existingApplication = await db.SavedJob.findUnique({
      where: {
        jobseekerId_jobId: {
    ...
    if (existingApplication) {
      return NextResponse.json(
        { error: "You have already saved this job", success: false },
        { status: 409 }
      );
    }
    ...
    return NextResponse.json(
      { result: "Job saved Successfully", success: true, savedjob },
      { status: 200 }
    );
  } catch (error) {
